    ...
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        for (let i = 0; i < count; i++) {
          setTimeout(() => setVisible((prev) => { const n = [...prev]; n[i] = true; return n; }), i * 100);
        }
        obs.disconnect();
      }
    }, { threshold: 0.1 });
...
/* ─── SVG Icons ─── */
const ico = {
  phone: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent1} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg>),
  star: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent3} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>),
  users: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent2} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 00-3-3.87" /><path d="M16 3.13a4 4 0 010 7.75" /></svg>),
  chart: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent1} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10" /><line x1="12" y1="20" x2="12" y2="4" /><line x1="6" y1="20" x2="6" y2="14" /></svg>),
  shield: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent2} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>),
  zap: (<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={P.accent3} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>),
...
const SERVICES = [
  { icon: ico.phone, title: "Never Miss a Patient", desc: "AI voice agent answers every call 24/7, books appointments into Google Calendar, sends WhatsApp confirmation + 2hr reminder. Missed call? WhatsApp in 60 seconds.", tag: "Core", color: P.accent1, tagBg: "#EFF6FF", tagBorder: "#BFDBFE" },
  { icon: ico.star, title: "Auto Review Engine", desc: "After every visit — 5-star patients get a Google review link. Negative feedback goes to you privately. Grow your Google rating 0.5–1 star in 60 days.", tag: "Growth", color: P.accent3, tagBg: "#FFF7ED", tagBorder: "#FED7AA" },
  { icon: ico.users, title: "Patient Reactivation", desc: "AI checks your CRM monthly for 6-month inactive patients, sends personalised WhatsApp, books them back. Recover 8–15 patients per month.", tag: "Revenue", color: P.accent2, tagBg: "#F5F3FF", tagBorder: "#DDD6FE" },
  { icon: ico.chart, title: "Owner Dashboard", desc: "Live Looker Studio dashboard: calls answered, appointments booked, missed calls recovered, reviews collected, and revenue attributed — all in one screen.", tag: "Insights", color: P.accent1, tagBg: "#EFF6FF", tagBorder: "#BFDBFE" },
  { icon: ico.shield, title: "Competitor Intel Brief", desc: "Weekly AI-generated report on your competing clinics delivered every Monday morning. Know what they're doing before they do.", tag: "Strategy", color: P.accent2, tagBg: "#F5F3FF", tagBorder: "#DDD6FE" },
  { icon: ico.zap, title: "Lead Qualification AI", desc: "Incoming leads verified in 60 seconds, qualified via WhatsApp conversation, scored 1–10. Hot leads get sent to your team with full context instantly.", tag: "Sales", color: P.accent3, tagBg: "#FFF7ED", tagBorder: "#FED7AA" },
];

const STEPS = [
  { num: "01", title: "Discovery Call", desc: "We map your missed calls, lost patients, and revenue gaps in a focused 20-minute call." },
  { num: "02", title: "System Build", desc: "We build your custom AI receptionist, WhatsApp flows, review engine, and dashboard in 5–7 days." },
  { num: "03", title: "Go Live & Grow", desc: "Your AI starts answering calls, recovering patients, and collecting reviews from day one. We optimise monthly." },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      else { setDeleting(false); setNicheIdx((i) => (i + 1) % NICHES.length); }
    }, speed);
  ...
  return (
    <>
      ...
      <div style={{ background: P.bg, minHeight: "100vh", color: P.text, fontFamily: FONT, overflowX: "hidden", position: "relative" }}>
          ...
          <div style={wrap}>
            <div style={{ opacity: heroReady ? 1 : 0, transform: heroReady ? "translateY(0)" : "translateY(30px)", transition: "all 1s cubic-bezier(0.23,1,0.32,1)" }}>
              ...
              <h1 className="hero-title" style={{ fontSize: "clamp(40px, 5.5vw, 68px)", fontWeight: 800, lineHeight: 1.06, letterSpacing: "-0.035em", color: P.textDark, maxWidth: 750, marginBottom: 20 }}>
                Your{" "}
                <span style={{ background: P.grad, backgroundSize: "200% 200%", animation: "gradShift 4s ease infinite", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                  {NICHES[nicheIdx].slice(0, charIdx)}
                  <span style={{ animation: "blink 1s step-end infinite", WebkitTextFillColor: P.accent2 }}>|</span>
                </span>
                <br />
                Never Misses a Call Again.
              </h1>

              <p style={{ fontSize: 18, lineHeight: 1.7, color: P.muted, maxWidth: 540, marginBottom: 36, fontWeight: 400 }}>
                We build AI voice agents that answer every patient call 24/7, book appointments, recover missed revenue, and grow your Google reviews — automatically.
              </p>

              <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                <CapsuleBtn primary href="/contact">Book Free 20-min Call {ico.arrow}</CapsuleBtn>
                <CapsuleBtn href="#services">See How It Works</CapsuleBtn>
              </div>
            </div>

            {/* STATS */}
            <div ref={statsAnim.ref} className="grid-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginTop: 72 }}>
              {STATS.map((s, i) => (
                <GlassCard key={i} style={{ padding: "26px 20px", textAlign: "center", opacity: statsAnim.visible[i] ? 1 : 0, transform: statsAnim.visible[i] ? "translateY(0)" : "translateY(20px)", transition: `all 0.6s cubic-bezier(0.23,1,0.32,1) ${i * 0.08}s` }}>
                  <div style={{ fontSize: 34, fontWeight: 800, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: "-0.03em" }}>{s.value}</div>
                  <div style={{ fontSize: 13, color: P.muted, marginTop: 6, fontWeight: 500 }}>{s.label}</div>
                </GlassCard>
              ))}
            </div>
          </div>
        ...
        {/* ═══ SERVICES ═══ */}
        <section id="services" style={{ position: "relative", zIndex: 1, padding: "80px 0" }}>
          <div style={wrap}>
            <Reveal>
              <div style={{ textAlign: "center", marginBottom: 56 }}>
                <p style={{ fontSize: 13, fontWeight: 700, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>The DentalOS Stack</p>
                <h2 style={{ fontSize: "clamp(30px, 4vw, 46px)", fontWeight: 800, letterSpacing: "-0.03em", color: P.textDark }}>5 Layers. Zero Missed Revenue.</h2>
                <p style={{ fontSize: 16, color: P.muted, marginTop: 14, maxWidth: 520, marginInline: "auto", lineHeight: 1.65 }}>Each layer compounds — the AI receptionist feeds reviews, reviews feed reactivation, reactivation feeds the dashboard.</p>
              </div>
            </Reveal>
            <div ref={servicesAnim.ref} className="grid-services" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {SERVICES.map((s, i) => (
                <GlassCard key={i} style={{ padding: "30px 26px", opacity: servicesAnim.visible[i] ? 1 : 0, transform: servicesAnim.visible[i] ? "translateY(0)" : "translateY(24px)", transition: "all 0.6s cubic-bezier(0.23,1,0.32,1)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    <div style={{ width: 46, height: 46, borderRadius: 13, background: `${s.color}0A`, border: `1px solid ${s.color}18`, display: "flex", alignItems: "center", justifyContent: "center" }}>{s.icon}</div>
                    <span style={{ fontSize: 11, fontWeight: 700, color: s.color, padding: "4px 12px", borderRadius: 999, background: s.tagBg, border: `1px solid ${s.tagBorder}`, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.tag}</span>
                  </div>
                  <h3 style={{ fontSize: 19, fontWeight: 700, marginBottom: 8, color: P.textDark, letterSpacing: "-0.01em" }}>{s.title}</h3>
                  <p style={{ fontSize: 14, lineHeight: 1.7, color: P.muted }}>{s.desc}</p>
                </GlassCard>
              ))}
            </div>
          </div>
        ...
        {/* ═══ HOW IT WORKS ═══ */}
        <section id="process" style={{ position: "relative", zIndex: 1, padding: "80px 0" }}>
          <div style={wrap}>
            <Reveal>
              <div style={{ textAlign: "center", marginBottom: 56 }}>
                <p style={{ fontSize: 13, fontWeight: 700, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>Process</p>
                <h2 style={{ fontSize: "clamp(30px, 4vw, 46px)", fontWeight: 800, letterSpacing: "-0.03em", color: P.textDark }}>Live in 7 Days. Not 7 Months.</h2>
              </div>
            </Reveal>
            <div ref={stepsAnim.ref} className="grid-steps" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {STEPS.map((s, i) => (
                <GlassCard key={i} style={{ padding: "36px 28px", textAlign: "center", opacity: stepsAnim.visible[i] ? 1 : 0, transform: stepsAnim.visible[i] ? "translateY(0)" : "translateY(24px)", transition: "all 0.6s cubic-bezier(0.23,1,0.32,1)" }}>
                  <div style={{ fontSize: 44, fontWeight: 800, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", marginBottom: 14, letterSpacing: "-0.04em" }}>{s.num}</div>
                  <h3 style={{ fontSize: 21, fontWeight: 700, marginBottom: 10, color: P.textDark }}>{s.title}</h3>
                  <p style={{ fontSize: 14, lineHeight: 1.7, color: P.muted }}>{s.desc}</p>
                </GlassCard>
              ))}
            </div>
          ...
          <div style={wrap}>
            <Reveal>
              <div style={{ textAlign: "center", marginBottom: 40 }}>
                <p style={{ fontSize: 13, fontWeight: 700, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>Industries We Serve</p>
            ...
            <div ref={industriesAnim.ref} className="grid-industries" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
              {INDUSTRIES.map((ind, i) => (
                <GlassCard key={i} style={{ padding: "22px 16px", textAlign: "center", opacity: industriesAnim.visible[i] ? 1 : 0, transform: industriesAnim.visible[i] ? "translateY(0)" : "translateY(16px)", transition: "all 0.5s cubic-bezier(0.23,1,0.32,1)" }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: P.textDark, marginBottom: 4 }}>{ind.name}</div>
                  <div style={{ fontSize: 11, color: P.muted, lineHeight: 1.5 }}>{ind.desc}</div>
                </GlassCard>
              ))}
            </div>
          </div>
        ...
        {/* ═══ PRICING ═══ */}
        <section id="pricing" style={{ position: "relative", zIndex: 1, padding: "80px 0" }}>
          <div style={wrap}>
            <Reveal>
              <div style={{ textAlign: "center", marginBottom: 48 }}>
                <p style={{ fontSize: 13, fontWeight: 700, background: P.grad, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>Pricing</p>
        ...
        {/* ═══ CTA ═══ */}
        <section id="contact" style={{ position: "relative", zIndex: 1, padding: "80px 0" }}>
          <div style={wrap}>
            <Reveal>
        ...
        {/* ═══ FOOTER ═══ */}
        <footer style={{ position: "relative", zIndex: 1, background: "rgba(255,255,255,0.5)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderTop: `1px solid ${P.border}`, padding: "48px 0 32px" }}>
          <div className="grid-footer" style={{ ...wrap, display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 48 }}>
            ...
            <div>
              ...
              {["AI Voice Agent", "Lead Nurturing", "WhatsApp Automation", "Review Engine", "Owner Dashboard"].map((s) => (
                <p key={s} style={{ fontSize: 14, color: P.muted, marginBottom: 10 }}>{s}</p>
              ))}
            </div>
            <div>
